  ...
  do {                                                                         \
    if (offset + step > length) {                                              \
      throw std::runtime_error(                                                \
          "offset overflow, offset: " + std::to_string(offset) + ",step: " +   \
          std::to_string(step) + " ,length: " + std::to_string(length));       \
    }                                                                          \
  } while (0)
...
bool WsMessage::encode(bcos::bytes &_buffer) {
  ...
  uint16_t type =
      boost::asio::detail::socket_ops::host_to_network_short(m_type);
  uint16_t status =
      boost::asio::detail::socket_ops::host_to_network_short(m_status);
...
ssize_t WsMessage::decode(const bcos::byte *_buffer, std::size_t _size) {
  ...
  // type field
  m_type =
      boost::asio::detail::socket_ops::network_to_host_short(*((uint16_t *)p));
  p += 2;
...
bool AMOPRequest::encode(bcos::bytes &_buffer) {
  ...
  uint16_t length = boost::asio::detail::socket_ops::host_to_network_short(
      (uint16_t)m_topic.size());
...
ssize_t AMOPRequest::decode(bytesConstRef _data) {
  ...
  try {
    ...
    // topic
    m_topic =
        std::string(_data.data() + offset, _data.data() + offset + topicLen);
    offset += topicLen;
    // data
