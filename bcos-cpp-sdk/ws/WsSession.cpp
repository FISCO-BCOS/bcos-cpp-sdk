...
void WsSession::disconnect() {
  try {
    ...
    m_wsStream.next_layer().socket().shutdown(
        boost::asio::ip::tcp::socket::shutdown_send, ec);
  } catch (const std::exception &e) {
    WEBSOCKET_SESSION(WARNING)
        << LOG_BADGE("disconnect") << LOG_KV("e", e.what());
  }
...
void WsSession::onRead(boost::beast::error_code _ec, std::size_t _size) {
  ...
  auto data = boost::asio::buffer_cast<bcos::byte *>(
...
void WsSession::asyncRead() {
  ...
  m_wsStream.async_read(m_buffer, std::bind(&WsSession::onRead, session,
                                            std::placeholders::_1,
                                            std::placeholders::_2));
}
...
void WsSession::addRespCallback(const std::string &_seq,
                                CallBack::Ptr _callback) {
  std::unique_lock lock(x_callback);
  m_callbacks[_seq] = _callback;
}

WsSession::CallBack::Ptr
WsSession::getAndRemoveRespCallback(const std::string &_seq) {
...
void WsSession::onRespTimeout(const boost::system::error_code &_error,
                              const std::string &_seq) {
  if (_error) {
    WEBSOCKET_SESSION(TRACE) << LOG_BADGE("onRespTimeout")
                             << LOG_KV("error", _error) << LOG_KV("seq", _seq);
    return;
  }
  ...
  auto errorPtr =
      std::make_shared<Error>(bcos::protocol::CommonError::TIMEOUT, "timeout");
  m_threadPool->enqueue([callback, errorPtr]() {
    callback->respCallBack(errorPtr, nullptr, nullptr);
  });
}