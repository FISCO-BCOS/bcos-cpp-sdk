 ...
 *
 * @file TransactionCodecInterface.h
 * @author: octopus
 * @date 2022-01-12
 */
#pragma once
...
namespace utilities
{
class TransactionCodecInterface
{
public:
    using Ptr = std::shared_ptr<TransactionCodecInterface>;

public:
    virtual void encode(
        const bcostars::TransactionData& _transactionData, bcos::bytes& _buffer) = 0;

    virtual void encodeAndSign(const bcostars::TransactionData& _transactionData, KeyPair _keyPair,
        bcos::bytes& _buffer) = 0;
    virtual void encodeAndSign(const bcostars::TransactionData& _transactionData, KeyPair _keyPair,
        std::string& _hexBuffer) = 0;
};
}  // namespace utilities
}  // namespace cppsdk
}  // namespace bcos