...
class WsSessionInitialize
    : public std::enable_shared_from_this<WsSessionInitialize> {
private:
...
public:
  ...
  explicit WsSessionInitialize(boost::asio::io_context &ioc)
      : m_resolver(boost::asio::make_strand(ioc)),
        m_wsStream(boost::asio::make_strand(ioc)) {}
  ...
  void run(char const *host, char const *port) {
    ...
    m_resolver.async_resolve(
        host, port,
        boost::beast::bind_front_handler(&WsSessionInitialize::on_resolve,
                                         shared_from_this()));
  }
  ...
  on_connect(boost::beast::error_code ec,
    ...
    m_wsStream.async_handshake(m_host, "/",
                               [this](boost::beast::error_code _ec) {
                                 if (_ec) {
  ...
  void
  setHandler(std::function<
             void(boost::beast::websocket::stream<boost::beast::tcp_stream> &&)>
                 _handler) {
    m_handler = _handler;
  }
...
int main(int argc, char **argv) {
  ...
  std::string message = std::string(1000, '1');
  if (argc > 4) {
    message = argv[4];
  }

  BCOS_LOG(INFO) << LOG_DESC("amop pub client sample") << LOG_KV("ip", host)
  ...
  initialize->setHandler(
      [wsService, topic,
       message](boost::beast::websocket::stream<boost::beast::tcp_stream>
                    &&_wsStream) {
        auto remoteEndPoint = _wsStream.next_layer().socket().remote_endpoint();
        std::string s = remoteEndPoint.address().to_string() + ":" +
                        std::to_string(remoteEndPoint.port());
        auto wsSession =
            std::make_shared<bcos::ws::WsSession>(std::move(_wsStream));
        ...
        auto pubThread = std::make_shared<std::thread>([wsSession, wsService,
                                                        topic, message]() {
          while (true) {
            wsService->publish(
                topic,
                std::make_shared<bcos::bytes>(message.begin(), message.end()),
                wsSession);
  ...
  BCOS_LOG(INFO) << LOG_DESC(" ==> public ") << LOG_KV("topic", topic);
